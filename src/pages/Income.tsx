...
const IncomePage: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="animate-pulse space-y-8">
      <div className="h-40 bg-white/5 rounded-[2.5rem]"></div>
  ...
  return (
    <div className="flex flex-col gap-10 animate-fade-in-up pb-10">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-4xl font-black tracking-tighter mb-2">{t('income.title')}</h1>
          <p className="text-white/40 font-medium">{t('income.description')}</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-card rounded-[2.5rem] p-10 border border-white/5">
            <h3 className="text-xl font-black mb-8">{t('income.recent_records')}</h3>
            <div className="space-y-4">
              {incomes.length === 0 ? (
                <div className="text-center py-20 text-white/10 font-black uppercase text-sm italic">
                  {t('income.no_records')}
                </div>
              ) : (
                incomes.map((income) => (
                  <div key={income.id} className="group flex items-center justify-between p-6 rounded-[2rem] hover:bg-white/[0.03] transition-all border border-transparent hover:border-white/5">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <div className="font-black text-white group-hover:text-green-500 transition-colors">{income.description || income.incomeSourceName}</div>
                        <div className="text-[10px] font-black uppercase tracking-widest text-white/20">{income.incomeSourceName} • {new Date(income.date).toLocaleDateString()}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-lg font-black text-green-400">+{formatCurrency(income.amount)}</div>
                      <div className="text-[9px] font-black uppercase tracking-tighter text-white/10">{income.accountName}</div>
                    </div>
                  </div>
                ))
              )}
